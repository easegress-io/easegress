...
const (
	...
	// StickySessionLBCookieExpireDefault is the default expire seconds of cookie generated by load balancer.
	StickySessionLBCookieExpireDefault = 7200
...
type StickySessionSpec struct {
	...
	// LBCookieName is the generated cookie name in CookieServerMatch mode, default is EG_SESSION.
	LBCookieName string `json:"lbCookieName" jsonschema:"omitempty"`
	// LBCookieExpire is the expire seconds of generated cookie in CookieServerMatch mode, default is 7200.
	LBCookieExpire int `json:"lbCookieExpire" jsonschema:"omitempty"`
}
...
func (blb *BaseLoadBalancer) initServerMatch() {
	...
	if blb.spec.StickySession.LBCookieExpire == 0 {
		blb.spec.StickySession.LBCookieExpire = StickySessionLBCookieExpireDefault
	}
}
...
func (blb *BaseLoadBalancer) chooseServerByServerMatch(req *httpprot.Request) *Server {
	...
	for _, s := range blb.Servers {
		if match(cookie.Value, []byte(s.ID())) {
			return s
		}
	}
...
func (blb *BaseLoadBalancer) ReturnServer(server *Server, req *httpprot.Request, resp *httpprot.Response) {
	...
	case StickySessionModeCookieServerMatch:
		cookie := &http.Cookie{
			...
			Expires: time.Now().Add(time.Duration(blb.spec.StickySession.LBCookieExpire * 1000000)),
		}
...
// match matches plain text byte array for encoded string
func match(encoded string, plain []byte) bool {
	signed, err := hex.DecodeString(encoded)
	if err != nil {
		return false
	}

	if len(signed) != KeyLen+sha256.Size {
		return false
	}
