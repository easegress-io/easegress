...
type (
	...
	runtime struct {
		...
		startNum  uint64           // runtime startEventLoop num
...
func (r *runtime) needRestartServer(nextSpec *Spec) bool {
	...
	x.TcpNodelay, y.TcpNodelay = true, true
...
func (r *runtime) startServer() {
	...
	if err := l.listen(); err != nil {
		...
		r.setError(err)
		logger.Errorf("listen for %s :%d failed, err: %+v", r.spec.Protocol, r.spec.Port, err)
...
func (r *runtime) onTcpAccept() func(conn net.Conn, listenerStop chan struct{}) {
	...
	return func(conn net.Conn, listenerStop chan struct{}) {
		...
		upstreamStopChan := make(chan struct{})
		upstreamConn := connection.NewUpstreamConn(connTimeout, serverAddr, listenerStop, upstreamStopChan)
		if err := upstreamConn.Connect(); err != nil {
			...
			cliConn := connection.NewClientConn(conn, conn.RemoteAddr(), listenerStop, clientStopChan)
			ctx := context.NewLayer4Context(cliConn, upstreamConn, conn.RemoteAddr())
			r.setOnReadHandler(cliConn, upstreamConn, ctx)

			// finish context lifecycle
			go func(chans ...chan struct{}) {
				for _, v := range chans {
					<-v
					ctx.Finish()
				}
			}(upstreamStopChan, clientStopChan)
		}
	}
}

func (r *runtime) onUdpAccept() func(cliAddr net.Addr, conn net.Conn, listenerStop chan struct{}, packet iobufferpool.IoBuffer) {
	return func(cliAddr net.Addr, conn net.Conn, listenerStop chan struct{}, packet iobufferpool.IoBuffer) {
		...
		if rawCtx, ok := connection.ProxyMap.Load(key); ok {
			ctx := rawCtx.(*connection.Connection)
			ctx.OnRead(packet)
			return
		}
		...
		connTimeout := time.Duration(r.spec.ProxyConnectTimeout) * time.Millisecond
		upstreamStopChan := make(chan struct{})
		upstreamConn := connection.NewUpstreamConn(connTimeout, upstreamAddr, listenerStop, upstreamStopChan)
		if err := upstreamConn.Connect(); err != nil {
		...
		connection.SetUDPProxyMap(connection.GetProxyMapKey(localAddr.String(), cliAddr.String()), &cliConn)
		r.setOnReadHandler(cliConn, upstreamConn, ctx)
...
func (r *runtime) setOnReadHandler(cliConn *connection.Connection, upstreamConn *connection.UpstreamConnection, ctx context.Layer4Context) {
	if handle, ok := r.mux.GetHandler(r.spec.Name); ok {
		cliConn.SetOnRead(func(buffer iobufferpool.IoBuffer) {
			_ = ctx.GetClientReadBuffer().Append(buffer.Bytes())
			handle.Handle(ctx) // its filter's response to clear context client read buffer
			buffer.Reset()
		})
		upstreamConn.SetOnRead(func(buffer iobufferpool.IoBuffer) {
			_ = ctx.GetUpstreamReadBuffer().Append(buffer.Bytes())
			handle.Handle(ctx) // its filter's response to clear context upstream read buffer
			buffer.Reset()
		})
	} else {
		cliConn.SetOnRead(func(buffer iobufferpool.IoBuffer) {
			ctx.DirectWriteToUpstream(buffer)
			buffer.Reset()
		})
		upstreamConn.SetOnRead(func(buffer iobufferpool.IoBuffer) {
			ctx.DirectWriteToClient(buffer)
			buffer.Reset()
		})
	}
}
