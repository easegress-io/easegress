...
// cleanExternalRegistry removes external service specs and service instances.
func (rs *registrySyncer) cleanExternalRegistry() {
	if !rs.needSync() {
		return
	}

	instances := rs.service.ListAllServiceInstanceSpecs()
	meshRegistryName := rs.meshRegistryName()
	externalRegistryName := rs.externalRegistryName()

	for _, instance := range instances {
		switch instance.RegistryName {
		case "", meshRegistryName, externalRegistryName:
			continue
		}

		rs.service.DeleteServiceInstanceSpec(instance.ServiceName, instance.InstanceID)
		logger.Infof("clean service instance: %s/%s", instance.ServiceName, instance.InstanceID)
	}

	specs := rs.service.ListServiceSpecs()
	for _, _spec := range specs {
		switch _spec.RegistryName {
		case "", meshRegistryName, externalRegistryName:
			continue
		}

		rs.service.DeleteServiceSpec(_spec.Name)
...
func (rs *registrySyncer) updateServiceSpec() {
	...
	globalServices := make(map[string]struct{})
	for _, service := range allServices {
		if service.RegisterTenant == spec.GlobalTenant {
			globalServices[service.Name] = struct{}{}
		}
		...
		if !exists {
			...
			delete(globalServices, service.Name)
		}
	}

	globalServicesSlice := make([]string, 0)
	for service := range externalServices {
		globalServicesSlice = append(globalServicesSlice, service)
	}
	sort.Strings(globalServicesSlice)
	rs.service.PutTenantSpec(&spec.Tenant{
		Name:        spec.GlobalTenant,
		Services:    globalServicesSlice,
	...
	for service := range externalServices {
		serviceSpec := &spec.Service{
			...
			Name:           service,
			RegisterTenant: spec.GlobalTenant,
