...
function buildbackend(){
    cd ${BACKENDDIR}
    go build urlratelimit.go
    if [ $? -ne 0 ];then
        echo "build urlratelimit error"
        return 1
    fi
    mv urlratelimit backend_urlratelimit
...
function runbackend(){
    cd ${BACKENDDIR}
    nohup ./backend_urlratelimit>${BACKENDDIR}/urlratelimit.res 2>&1 &
    if [ $? -ne 0 ];then
...
function termbackend(){
    PID=`ps -ef|grep backend_urlratelimit|grep -v grep|awk '{print $2}'`
    if [ "$PID" != "" ];then
...
function createobject(){
    cd ${CONFIGDIR}
    rm -f generate*.yaml
    grep "^name:" *ratelimit*.yaml|sed -E 's/.*name: (.*)/\1/'|while read objname
    do
...
function checkobject(){
    curl -v ${TESTHOST}:10080/urlretelimit
    if [ $? -ne 0 ];then
...
function testobject(){
    cd ${BUILDDIR}/scripts
    ./autotest-cotest-urlratelimit.sh
    if [ $? -ne 0 ];then
        return 1
    fi
    echo "test success"
