...
function buildbackend(){
    cd ${BACKENDDIR}
    go build circuitbreaker.go
    if [ $? -ne 0 ];then
        echo "build circuitbreaker error"
        return 1
    fi
    mv circuitbreaker backend_circuitbreaker
    echo "build circuitbreaker successed."
    return 0
}

function runbackend(){
    cd ${BACKENDDIR}
    nohup ./backend_circuitbreaker>${BACKENDDIR}/circuitbreaker.res 2>&1 &
    if [ $? -ne 0 ];then
        echo "run circuitbreaker error"
        return 1
    fi
    return 0
}
...
function termbackend(){
    PID=`ps -ef|grep backend_circuitbreaker|grep -v grep|awk '{print $2}'`
    if [ "$PID" != "" ];then
        echo $PID|xargs kill -9
    fi
    echo "term circuitbreaker successed."
    return 0
}
...
function createobject(){
    cd ${CONFIGDIR}
    rm -f generate*.yaml
    grep "^name:" circuitbreaker*.yaml|sed -E 's/.*name: (.*)/\1/'|while read objname
    do
...
function checkobject(){
    ...
    if [ $? -ne 0 ];then
        echo "check object circuitbreaker error"
        return 1
    fi
...
function testobject(){
    cd ${BUILDDIR}/scripts
    ./autotest-cotest-circuitbreaker.sh
    if [ $? -ne 0 ];then
        return 1
    fi
    echo "test success"
