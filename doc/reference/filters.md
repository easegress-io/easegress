...
| sticky | bool   | Whether to use sticky sessions | No       |
| stickyCookie  | string | When `sticky` is `true`, this option is used to set name of app cookie, if empty, load balander will generate a new cookie | No       |
| stickyExpire  | string | When `sticky` is `true` and `stickyCookie` is not set, this option is used to set expire time of cookie generated by load balancer, default is session | No       |
