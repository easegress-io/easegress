...
| stickinessEnabled | bool   | Whether to use sticky sessions | No       |
| stickinessAppCookieName  | string | When `stickinessEnabled` is `true`, this option is used to set name of app cookie, if empty, load balander will generate a new cookie | No       |
| stickinessLbCookieExpire  | string | When `stickinessEnabled` is `true` and `stickinessAppCookieName` is not set, this option is used to set expire time of cookie generated by load balancer, default is session | No       |
