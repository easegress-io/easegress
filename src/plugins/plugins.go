...
// 2. Should be implemented as stateless, an plugin instance could be used in
//    different pipeline or parallel running instances of same pipeline.
//    Under current implementation, a plugin couldn't be used in different pipeline
//    but there's no guarantee this limitation is existing in future release.
// 3. Prepare(pipelines.PipelineContext) guarantees it will be called on the same
//    pipeline context against the same plugin instance only once before executing
//    Run(task.Task) on the pipeline.
