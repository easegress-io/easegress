...
func (gc *GatewayCluster) issueStat(group string, timeout time.Duration, detail bool,
	requestName string, filter Stater) (StatResulter, *ClusterError) {
	req := &ReqStat{
		Detail: detail,
		Stat:   filter,
	}
...
func (gc *GatewayCluster) statResult(s Stater) (StatResulter, error, ClusterErrorType) {
...
func (gc *GatewayCluster) handleStatRelay(req *cluster.RequestEvent) {
	...
	if err, errType = reqStat.Stat.Validate(); err != nil {
		logger.Warnf("[BUG: reqStat.Stat is not valid %v]", err)
		return
	}
...
func (gc *GatewayCluster) handleStat(req *cluster.RequestEvent) {
	...
	aggregatedPayload, err := localResp.Payload.Aggregate(reqStat.Detail, otherRespPayloads)
	if err != nil {
		logger.Warnf("aggregate statistics for cluster members failed: %v", err)
		gc.respondStatErr(req, InternalServerError,
			fmt.Sprintf("aggregate statistics for cluster members failed: %v", err))
		return
	}

	logger.Warnf("Aggregated respStat.Payload : %+v", aggregatedPayload)
