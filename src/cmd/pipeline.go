...
// NOTICE: the reason why not reuse same code of plugin, because in the future
// we must add different arguments in plgin and pipeline.

func pipelineAdd(c *cli.Context) error {
	args := c.Args()

	errs := &multipleErr{}

	postOnce := func(source, data string) {
		resp, err := post("/admin/v1/pipelines", data)
		if err != nil {
...
func pipelineDelete(c *cli.Context) error {
	args := c.Args()
	...
	deleteOnce := func(source string) {
		resp, err := delete_("/admin/v1/pipelines/" + source)
		if err != nil {
			errs.append(fmt.Errorf("%s: %s", source, err))
			return
		}
...
func pipelineGet(c *cli.Context) error {
	args := c.Args()

	errs := &multipleErr{}

	getAll := func() (string, error) {
		resp, err := get("/admin/v1/pipelines", "{}")
		if err != nil {
	...
	getOnce := func(source string) (string, error) {
		resp, err := get("/admin/v1/pipelines/"+source, "{}")
		if err != nil {
...
func pipelineUpdate(c *cli.Context) error {
	args := c.Args()

	errs := &multipleErr{}

	putOnce := func(source, data string) {
		resp, err := put("/admin/v1/pipelines", data)
		if err != nil {
			errs.append(fmt.Errorf("%s: %s", source, err))
			return
		}
